...
class VnStat extends \yii\db\ActiveRecord
{
    ...
    public static function getovst(){
        $model = new VnStat;
        return $model;
    }
    /**
  ...
  public function getPay($id){
      $model = RcptArrear::find()->where(['vn'=>$id])->one();
      $pay = @$model->amount;
      return $pay;
  }
  public function getAmount($id){
      $model= Opitemrece::find()->select('sum(sum_price) as amount')->where(['vn'=>$id])->groupBy('vn')->one();
      $amount = @$model->amount;
      return $amount;
  }
 
 public function getType($id){
     $model = Opitemrece::find()->select('count(*) as cnt')->where(['vn'=>$id])->one();
     
     $check = $model->cnt > 0 ? 'OPD' : 'IPD';
