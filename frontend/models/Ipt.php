 ...
 * @property string $chart_stat
 * @property string $rcv_date
 * @property string $rcv_from
...
class Ipt extends \yii\db\ActiveRecord
{
    ...
    public function rules()
    {
        return [
            [['an'], 'required'],
            [['dchdate', 'dchtime', 'regdate', 'regtime', 'receive_chart_date_time', 'last_check_autoincome', 'finance_summary_date', 'estimate_discharge_date', 'rcv_date', 'update_datetime'], 'safe'],
            ...
            [['rfrics', 'rfrocs', 'result', 'an_lock', 'ergent', 'chart_state', 'finance_lock', 'operation_status', 'old_cause_revisit', 'finance_transfer', 'no_visit', 'no_food', 'confirm_discharge', 'lab_status', 'xray_status'], 'string', 'max' => 1],
            ...
            [['chart_stat'], 'string', 'max' => 6],
            [['rcv_from'], 'string', 'max' => 30],
    ...
    public function attributeLabels()
    {
        ...
        public function getSummery($an){
        
        $model = \frontend\models\Opitemrece::find()->select('sum(sum_price) as amount')->where(['an'=>$an])->groupBy('an')->one();
        $amount = @$model->amount;
